...
function getCalculators() {
	calculators += 1;
	totalCalculators.innerHTML = parseInt(calculators).toLocaleString() + " Calculators";
	makeVisible();
}
...
function getSeller() {
	if (calculators >= nextSeller) {
		...
		cPS = cPS.toFixed(1);
		perSecond.innerHTML = "(" + cPS.toLocaleString() + " calculators per second)";
	}
}
